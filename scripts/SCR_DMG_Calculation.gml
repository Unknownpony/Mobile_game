...
if(is_array(target) == 1)
{
    ...
    {
        target[dc_i].hp -= floor((player.ad * (100 - target[dc_i].ar)/100) + (player.md * (100 - target[dc_i].mr)/100))
...
{
    target.hp -= floor((player.ad * (100 - target.ar)/100) + (player.md * (100 - target.mr)/100))
    if(target.debuff == 0)
        switch (player.effect)
        {
            case 1: 
